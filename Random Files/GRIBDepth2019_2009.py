...
# #Read the GRIB File
grbs = pygrib.open('C:/Users/frumkina/Desktop/Python/snow_2008_2001_era5land.grib')
# #grb = grbs.message(8760)
# #print(grb)
...
Data_File = open("2008_2001_Depth.txt","w")
list = [("Date,","Time,","Snow_Depth")]
for count in range(70128,1,1):
    print(count)
    grb = grbs.message(count)
    date_time = grb.validDate.strftime("%m/%d/%Y,%H:%M:%S,")
    data,lats,lons = grb.data()
#    print(type(data))
    list.append((date_time,float(data[4,1]*100),"\n"))
    Data_File.write(date_time + str(round(float(data[4,1]*100),1)) + "\n")
