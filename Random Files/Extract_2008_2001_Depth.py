...
#Read the GRIB File
grbs = pygrib.open('snow_2008_2001_era5land.grib')
...
data2 = grb.data()
data3 = grb.data()[0]
data4 = round(data3[4,4]*100,1)
latitude = round(lats[4,4],1)
longitude = round(lons[4,4],1)
#a = np.array([[1, 2, 3], [4, 5, 6]])
#newArray = np.append(a, [[50, 60, 70]], axis = 0)
#print(newArray)

#now we will try to loop through multiple steps
Data_File = open("2008_2001_Center_London.txt","w")
list = [("Date,","Time,","Snow_Depth")]
for count in range(1,70129,1):
    print(count)
